...
class HotelReservation(models.Model):
    ...
    reservation_line_ids = fields.One2many(
        ...
        "line_id",
        "Reservation Line",
        help="GoPuppy room reservation details.",
        readonly=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    def check_reservation_rooms(self):
        ...
        for reservation in self:
            ...
            for rec in reservation.reservation_line_ids:
                if not rec.reserve:
                    raise ValidationError(
                        _("Please Select Vans For Reservation.")
                    )
            ...
            if not ctx.get("duplicate"):
                if (reservation.adults + reservation.children) > cap:
                    raise ValidationError(
                        _(
                            "Van Capacity Exceeded \n"
                            " Please Select Vans According to"
                            " Members Accomodation."
                        )
                    )
            if reservation.adults <= 0:
                raise ValidationError(
                    _("Number of Animals must be Positive value.")
                )
...
class HotelRoomReservationLine(models.Model):
    ...
    room_id = fields.Many2one("hotel.room", string="Van id")
    check_in = fields.Datetime("Check In Date", required=True)
    check_out = fields.Datetime("Check Out Date", required=True)
    state = fields.Selection(
        [("assigned", "Assigned"), ("unassigned", "Unassigned")], "Van Status"
    )
