...
class HotelFloor(models.Model):
    ...
    name = fields.Char("VanShelf Name", required=True, index=True)
...
class HotelRoom(models.Model):
    ...
    room_categ_id = fields.Many2one(
        "hotel.room.type", "Van Category", required=True, ondelete="restrict"
    )
    ...
    room_line_ids = fields.One2many(
        "folio.room.line", "room_id", string="Van Reservation Line"
    )
    ...
    def check_capacity(self):
        for room in self:
            if room.capacity <= 0:
                raise ValidationError(_("Van capacity must be more than 0"))
